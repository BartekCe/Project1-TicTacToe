...
public class Game {
    ...
    public String checkGameWinner(char [][]grid){
        String result = "None";
            ...
            for(int i = 0; i<3; i++) {
                if (this.grid[i][0] == 'x' && this.grid[i][1] == 'x' && this.grid[i][2] == 'x') { // check X for vertical wins
                    result = "x wins";
                } else if (this.grid[0][i] == 'x' && this.grid[1][i] == 'x' && this.grid[2][i] == 'x') { // check X for horizontal wins
                    result = "x wins";
                } else if (this.grid[i][0] == 'o' && this.grid[i][1] == 'o' && this.grid[i][2] == 'o') { // check X for vertical wins
                    result = "o wins";
                } else if (this.grid[0][i] == 'o' && this.grid[1][i] == 'o' && this.grid[2][i] == 'o') { // check O for horizontal wins
                    result = "o wins";
                } else if (this.grid[1][1] == 'x' && ((this.grid[0][0] == 'x' && this.grid[2][2] == 'x') ||
                        (this.grid[0][2] == 'x' && this.grid[2][0] == 'x'))) { result = "x wins"; }     // Check X for diagonal wins
                else if (this.grid[1][1] == 'o' && ((this.grid[0][0] == 'o' && this.grid[2][2] == 'o') ||
                        (this.grid[0][2] == 'o' && this.grid[2][0] == 'o'))) { result = "o wins"; } // Check O for diagonal wins

                else if (this.grid[0][0] != '-' && this.grid[0][1] != '-' && this.grid[0][2] != '-' &&
                        this.grid[1][0] != '-' && this.grid[1][1] != '-' && this.grid[1][2] != '-' &&
                        this.grid[2][0] != '-' && this.grid[2][1] != '-' && this.grid[2][2] != '-' && result.equals("None")) {
                    result = "Tie"; //check if tie
                }
            }
        return result;
    }
